...
export class CanvasRenderer {
    ...
    private drawSelectedPieceOutline(piece: PuzzlePiece): void {
        this.ctx.save();
    ...
    drawConnectionIndicators(
        ...
        canConnect: (a: PuzzlePiece, b: PuzzlePiece) => boolean
    ): void {
    ...
    drawConnectionPreview(
        ...
        for (const otherPiece of pieces) {
            ...
            for (const point of connectionPoints) {
                if (draggedPiece.canConnect(otherPiece, point as ConnectionPoint)) {
                    this.ctx.save();
                    this.ctx.strokeStyle = '#e74c3c';
                    this.ctx.setLineDash([5, 5]);
                    this.ctx.beginPath();
                    ...
                    this.ctx.lineTo(
                        draggedPiece.x + draggedPiece.width / 2,
                        draggedPiece.y + draggedPiece.height / 2
                    );
                    this.ctx.stroke();
                    this.ctx.restore();
                    break;
