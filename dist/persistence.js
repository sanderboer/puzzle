...
export class PersistenceError extends Error {
    constructor(message, shouldDiscard = false) {
        super(message);
...
export class SaveManager {
    ...
    performSave(reason, gameData) {
        try {
            const serialized = serializeGameState(gameData.pieces, gameData.pieceGroups, gameData.image, gameData.gridCols, gameData.gridRows, gameData.timer);
...
export function serializeGameState(pieces, pieceGroups, image, gridCols, gridRows, timer) {
    const now = Date.now();
    ...
    const piecesData = pieces.map((piece, index) => {
        const group = pieceGroups.get(piece);
        let groupId;
        if (group) {
